...
def test_la_guardia_de_arrancar_no_comparte_cliente_con_la_web():
    """Compartirlo le descontaba a la guardia todo lo que pidieras desde la página.

    El tope de la guardia se mide con el contador de peticiones del cliente.
    Si es el mismo que usa el servidor web, abrir una previa desde el móvil le
    gasta presupuesto a la guardia y se corta sola sin motivo.
    """
    import inspect

    from cancha.comandos import servicio

    fuente = inspect.getsource(servicio.cmd_arrancar)
